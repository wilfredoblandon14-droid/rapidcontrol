...
export default function VistaMotorizado() {
  ...
  useEffect(() => {
    async function cargarDatos() {
      ...
      const listaMotorizados =
        (respuestaMotorizados.data ??
          []) as Motorizado[];

      setMotorizados(listaMotorizados);

      setPedidos(
        (respuestaPedidos.data ?? []) as Pedido[]
      );
      ...
      const idGuardado = seleccionGuardada
        ? Number(seleccionGuardada)
        : null;

      const motorizadoExiste =
        idGuardado !== null &&
        listaMotorizados.some(
          (motorizado) =>
            motorizado.id === idGuardado
        );

      if (motorizadoExiste) {
        setMotorizadoSeleccionado(idGuardado);
      }
    ...
    <main className="min-h-screen bg-slate-950 p-4 text-white sm:p-6">
      <div className="mx-auto max-w-3xl">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link
            href="/"
            className="w-fit rounded-xl border border-slate-700 px-5 py-3 font-bold transition hover:bg-slate-800"
          >
            ← Dashboard
          </Link>
        </header>
