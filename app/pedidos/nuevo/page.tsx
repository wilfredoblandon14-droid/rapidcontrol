...
export default function NuevoPedido() {
  ...
  async function guardarPedido(event: FormEvent<HTMLFormElement>) {
    ...
    const nuevoPedido = {
      nombre_cliente: formulario.get("nombre_cliente")?.toString().trim(),
      telefono: formulario.get("telefono")?.toString().trim(),
      tipo_servicio: formulario.get("tipo_servicio")?.toString(),
      metodo_pago: formulario.get("metodo_pago")?.toString(),
      direccion_recogida: formulario
        .get("direccion_recogida")
        ?.toString()
        .trim(),
      direccion_entrega: formulario
      ...
      motorizado_id:
        motorizadoSeleccionado && motorizadoSeleccionado !== ""
          ...
          : null,
      estado: formulario.get("estado")?.toString(),
      descripcion: formulario.get("descripcion")?.toString().trim(),
      observaciones: formulario.get("observaciones")?.toString().trim(),
    };
    ...
    if (errorSupabase) {
      ...
      setError(`No se pudo guardar: ${errorSupabase.message}`);
      setGuardando(false);
      return;
    }
        ...
        >
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Campo etiqueta="Motorizado">
              <select name="motorizado_id" className="campo">
                <option value="">Sin asignar</option>
                <option value="1">Carlos López</option>
                <option value="2">Luis Hernández</option>
                <option value="3">Kevin Ramírez</option>
                <option value="4">Miguel Díaz</option>
              </select>
            </Campo>
          ...
          {mensaje && (
            <p className="mt-6 rounded-xl bg-green-950 p-4 text-green-400">
              ✅ {mensaje}
            </p>
          )}

          {error && (
            <p className="mt-6 rounded-xl bg-red-950 p-4 text-red-400">
              ❌ {error}
            </p>
          )}
